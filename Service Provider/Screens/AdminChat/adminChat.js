...
import { addAdminMsg } from '../../store/action/action'
...
class AdminChat extends Component {
    ...
    componentDidMount() {
        const { navigation, adminMsg, flag } = this.props

        const data = navigation.getParam('data')
        this.setState({ data })
        console.log(data, 'datat herer')
        if (adminMsg && adminMsg.length || flag || !flag) {
            adminMsg.sort((a, b) => {
                return a.time - b.time
            })
            this.setState({ adminMsg })
            console.log(adminMsg, 'adminMsgmount')
        }
    }

    componentWillReceiveProps(props) {
        const { navigation, adminMsg, flag } = props
    ...
    messageSend() {
        ...
        if (text) {
            ...
            const { addAdminMsg } = this.props.actions
            var obj = {
                sender: userUid,
    ...
    render() {
        const { text, data, adminMsg } = this.state
        return (
